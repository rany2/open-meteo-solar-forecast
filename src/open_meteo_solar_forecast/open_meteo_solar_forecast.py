    ...
    async def estimate(self) -> Estimate:
        ...
        for az, dec, dc_kwp, lat, lon, eff in zip(
            ...
            for i, time in enumerate(time_arr):
                ...
                w_avg[time_start] += gen_power(g_avg, temp_avg, eff)
                w_inst[time_start] += gen_power(g_inst, temp_inst, eff)

                # Apply the damping factor
                w_avg[time_start] *= damping_factors[i]
                w_inst[time_start] *= damping_factors[i]
