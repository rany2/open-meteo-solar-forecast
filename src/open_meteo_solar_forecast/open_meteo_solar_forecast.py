...
class OpenMeteoSolarForecast:
    ...
    async def estimate(self) -> Estimate:
        ...
        def gen_power(
            *,
            ...
            ---------
                ...
                CCF=1-a*cc^b
                CCF=1-0.75*cc^3.4 (Kasten-Czeplak fitting)
                Currently using a different fitting, but the same principle.
                Source: https://www.sciencedirect.com/science/article/abs/pii/0038092X80903916
            """

            def hw_v(v: float) -> float:
                return 8.91 + 2.0 * v

            def ccf(cc: float) -> float:
                a, b = 0.75, 3.4
                return 1 - a * cc**b

            gti *= ccf(cc)
            temp_cell = gti / G_NOCT
            temp_cell *= hw_v(WIND_NOCT_SPEED) / hw_v(ws)
        ...
        for az, dec, dc_kwp, lat, lon, eff in zip(
            ...
            # As Open-Meteo doesn't specify what the max altitude for high is,
            # we'll assume that the max altitude for high is 13km.
            #
            # The cloud cover is split into three categories:
            #  low: 0km -> 3km (3/13 = 0.23)
            #  mid: 3km -> 8km (5/13 = 0.38)
            #  high: 8km -> 13km (5/13 = 0.38)
            #  total: 0km -> 13km
            cc_arr = [
                (
                    (cc_low / 100) * 0.23 + (cc_mid / 100) * 0.38
                    if cc_low is not None and cc_mid is not None
                    else None
                )
                for cc_low, cc_mid in zip(
                    data["minutely_15"]["cloudcover_low"],
                    data["minutely_15"]["cloudcover_mid"],
                    strict=True,
                )
            ]
            ...
            for i, time in enumerate(time_arr):
                ...
                # Get the temp, wind speed, and cloud cover for avg and inst values
                t_avg = (temp_arr[i] + temp_arr[i - 1]) / 2
                ws_avg = (wind_arr[i] + wind_arr[i - 1]) / 2
                cc_avg = (cc_arr[i] + cc_arr[i - 1]) / 2
                t_inst = temp_arr[i - 1]
                ws_inst = wind_arr[i - 1]
                cc_inst = cc_arr[i - 1]
